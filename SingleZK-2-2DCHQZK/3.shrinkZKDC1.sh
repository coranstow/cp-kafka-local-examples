...
for i in {1,2}
  do docker-compose -f docker-compose-step0.yml -f docker-compose-step1.yml -f docker-compose-step3.yml up -d kafka_$i
  echo "Sleeping $DELAY"
  sleep $DELAY
done
...
for i in {1,2,3,6,7,8}
  do docker-compose -f docker-compose-step0.yml -f docker-compose-step1.yml -f docker-compose-step3.yml up -d zookeeper_$i
  echo "Sleeping $DELAY"
  sleep $DELAY
done
